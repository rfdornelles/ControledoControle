...
```{r chama-analises, message=FALSE, warning=FALSE, include=FALSE}
source("R/8-analises.R", encoding = "UTF-8")

```


```{r grafico_distribuicao_acoes}

grafico_distribuicao_acoes
```

```{r grafico_acoes_classe}

grafico_acoes_classe
```

```{r grafico_partes_categorizadas}

grafico_partes_categorizadas
```


```{r grafico_quem_covid}

grafico_quem_covid
```

```{r grafico_evolucao_acoes_covid}
